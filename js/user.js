...
// اضافه کردن تابع برای ارسال کد تایید ایمیل
async function sendEmailVerificationCode() {
    try {
        const { error } = await supabase.auth.resend({
            ...
            options: {
                emailRedirectTo: `${window.location.origin}/user.html`
            }
        });
...
// اضافه کردن تابع برای تایید کد
async function verifyEmailCode(code) {
    // در اینجا باید منطق تایید کد پیاده‌سازی شود
    // این یک پیاده‌سازی ساده است
    return code.length === 6 && /^\d+$/.test(code);
}

// اصلاح تابع updatePersonalInfo برای نیاز به تایید ایمیل
async function updatePersonalInfo(e) {
    e.preventDefault();
    ...
    try {
        ...
        // اگر ایمیل تغییر کرده، نیاز به تایید دارد
        if (email !== userData.email) {
            // نمایش مودال تایید ایمیل
            document.getElementById('email-verification-modal').style.display = 'block';
            window.pendingProfileUpdate = { name, email };
...
// اضافه کردن تابع برای تایید کد و به‌روزرسانی اطلاعات
async function verifyAndUpdateProfile() {
    try {
        ...
        const updateData = window.pendingProfileUpdate;
        ...
        // به‌روزرسانی اطلاعات در localStorage
        userData.full_name = updateData.name;
...
// در تابع setupEventListeners، لیسنرهای جدید اضافه کنید
function setupEventListeners() {
    // ... کدهای موجود ...
    
    // اضافه کردن لیسنر برای تایید کد پروفایل
    document.getElementById('verify-profile-code').addEventListener('click', verifyAndUpdateProfile);
    document.getElementById('resend-profile-code').addEventListener('click', async () => {
        await sendEmailVerificationCode();
